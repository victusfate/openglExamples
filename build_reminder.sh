...
g++ $(pkg-config --cflags glfw3) $(pkg-config --cflags glew) $(pkg-config --cflags GraphicsMagick++) -o main main.cc $(pkg-config --libs glfw3) $(pkg-config --libs glew) $(pkg-config --libs GraphicsMagick++) -framework OpenGL -framework Cocoa
